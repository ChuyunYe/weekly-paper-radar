...
def fetch_arxiv(keywords, since):
    ...
    for entry in root.findall("a:entry", ns):
        ...
        papers.append({"title": clean(entry.findtext("a:title", default="", namespaces=ns)), "authors": [clean(x.findtext("a:name", default="", namespaces=ns)) for x in entry.findall("a:author", ns)], "published": published, "abstract": clean(entry.findtext("a:summary", default="", namespaces=ns)), "url": "https://arxiv.org/abs/" + ident, "arxiv_id": ident, "doi": None, "sources": ["arXiv"], "cited_by_count": 0})
    return papers

def fetch_openalex(keywords, since):
    search = " OR ".join(keywords[:8])
    url = "https://api.openalex.org/works?" + urllib.parse.urlencode({"search": search, "filter": "from_publication_date:" + since.isoformat(), "per-page": 50, "select": "title,authorships,publication_date,abstract_inverted_index,doi,primary_location,cited_by_count"})
...
def render(week, grouped, site_name, refresh_url):
    ...
    for topic, papers in grouped:
        ...
        for p in papers:
            ...
            cards.append('<article class="paper">%s<button class="save-paper" data-paper="%s" type="button">Save paper</button><h3><a href="%s" target="_blank" rel="noreferrer">%s</a></h3><p class="meta">%s · %s · %s</p><p><b>Research question:</b> %s</p><p><b>Method / content:</b> %s</p><p><b>Contribution:</b> %s</p><p><b>Reading note:</b> %s</p><p><b>Why read it:</b> %s</p></article>' % (featured,payload,html.escape(p["url"],quote=True),html.escape(p["title"]),html.escape(authors),html.escape(p["published"]),html.escape(" / ".join(p["sources"])),html.escape(s["question"]),html.escape(s["method"]),html.escape(s["contribution"]),html.escape(s["caveat"]),html.escape(s["why_read"])))
