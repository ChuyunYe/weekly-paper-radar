...
def render(week, grouped, site_name, refresh_url):
    ...
    for topic, papers in grouped:
        ...
        for p in papers:
            ...
            cards.append('<article class="paper">%s<button class="save-paper" data-paper="%s" type="button">Save paper</button><h3><a href="%s" target="_blank" rel="noreferrer">%s</a></h3><p class="meta">%s · %s · %s</p><p><b>Authors & affiliations:</b><br>%s</p><p><b>Keywords:</b> %s</p>%s<details><summary>Abstract</summary><p>%s</p></details><p><b>Research question:</b> %s</p><p><b>Method / content:</b> %s</p><p><b>Contribution:</b> %s</p><p><b>Reading note:</b> %s</p><p><b>Why read it:</b> %s</p></article>' % (featured,payload,html.escape(p["url"],quote=True),html.escape(p["title"]),html.escape(authors),html.escape(p["published"]),html.escape(" / ".join(p["sources"])),render_authors(p),keywords_html,journal,html.escape(p.get("abstract") or "The source did not provide an abstract."),html.escape(s["question"]),html.escape(s["method"]),html.escape(s["contribution"]),html.escape(s["caveat"]),html.escape(s["why_read"])))
